...
def try_set_version_from_env(version_file):
    opencv_version = os.environ.get('OPENCV_PYTHON_VERSION')
    if not opencv_version:
        raise RuntimeError(f"missing {version_file} and .git folder, aborting build")
    with open(version_file, 'w') as fp:
        print(f'opencv_version = {opencv_version!r}', file=fp)
        print('contrib = False', file=fp)
        print('headless = False', file=fp)
        print('rolling = False', file=fp)
        print('ci_build = False', file=fp)
...
def get_and_set_info(contrib, headless, rolling, ci_build):
    ...
    if not os.path.exists(version_file):
        try_set_version_from_env(version_file)
